...
const tierAccent: Record<AntipastoItem['tier'], string> = {
  base: 'from-cream/[0.04]',
  mid: 'from-gold/[0.07]',
  premium: 'from-burgundy/25',
};
...
export function AntipastoCard({ board, className }: { board: AntipastoItem; className?: string }) {
  ...
  return (
    <article
      className={cx(
        'group relative flex h-full flex-col overflow-hidden rounded-sm border border-cream/10',
        'bg-ink-800/70 p-6 transition-colors duration-400 hover:border-cream/25',
        className,
      )}
    >
      <div
      ...
      <header className="relative">
        <p className="eyebrow text-gold">{board.pairing}</p>
      ...
      <ul className="relative mt-7 flex-1 space-y-2.5">
        {board.contents.map((entry) => (
          <li key={entry} className="flex items-baseline gap-3 text-[0.875rem] text-cream-dim/80">
            <span aria-hidden="true" className="h-px w-3 shrink-0 translate-y-[-0.25rem] bg-gold/40" />
            {entry}
          </li>
        ))}
      </ul>

      <footer className="relative mt-8 flex items-baseline justify-between border-t border-cream/8 pt-5">
